...
# Foi colocado os dados duas vezes para já vir selecionado, mas pode ser retirado um deles para não vir selecionado. senão iria aaparecer vazio, mas fica a critério do usuário.
def display_dataframe(data):
    st.header("Visualização do DataFrame")
    st.sidebar.header("Filtros")
    selected_region = st.sidebar.multiselect(
        "Selecione as regiões", data["Regiao"].unique(), data["Regiao"].unique()
    )
